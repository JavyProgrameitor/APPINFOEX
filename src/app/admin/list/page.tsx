    ...
    <div className="p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
        <h1 className="text-xl md:text-2xl font-semibold">
          Listado por Zona: Jefe Servicio-- Bombero.F
        </h1>
      ...
      <Card>
        <CardHeader className="gap-2">
          ...
          <div className="flex flex-col md:flex-row gap-2 md:items-center">
            ...
            <div className="text-sm text-muted-foreground">
              {!zona && 'Elige una zona'}
              {zona && scope === 'unidad' && !unidadId && (
                <>
                  {' '}
                  · Zona <span className="font-medium">{zona}</span> · Elige una unidad
                </>
              )}
              {zona && scope === 'unidad' && !!unidadId && (
                <>
                  {' '}
                  · Zona <span className="font-medium">{zona}</span> · Unidad{' '}
                  <span className="font-medium">{unidadById.get(unidadId)?.nombre}</span>
                </>
              )}
              {zona && scope === 'caseta' && !casetaId && (
                <>
                  {' '}
                  · Zona <span className="font-medium">{zona}</span> · Elige una caseta
                </>
              )}
              {zona && scope === 'caseta' && !!casetaId && (
                <>
                  {' '}
                  · Zona <span className="font-medium">{zona}</span> · Caseta{' '}
                  <span className="font-medium">{casetaById.get(casetaId)?.nombre}</span>
                </>
              )}
            </div>
          </div>
        ...
        <CardContent>
          {!zona || (scope === 'unidad' && !unidadId) || (scope === 'caseta' && !casetaId) ? (
            <div className="text-sm text-muted-foreground">
              Selecciona zona y luego {scope === 'unidad' ? 'una unidad' : 'una caseta'} para ver el
              listado.
            </div>
          ) : loading ? (
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                            {unidad?.zona ||
                              (caseta ? municipioById.get(caseta.municipio_id)?.zona : '—')}
                          </div>
                          {/* Chip de rol */}
            ...
            <div className="p-4 space-y-4">
              <div>
                <div className="text-xl font-semibold">
                  {selected.apellidos}, {selected.nombre}
                </div>
                <div className="text-xs text-muted-foreground">
                  Usuario :{' '}
                  <span className="font-medium">
                    {selected.rol === 'jr' ? 'Jefe de Servicio' : 'Bombero Forestal'}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground">
                  Creado el {new Date(selected.creado_en).toLocaleDateString()}
                </div>
              </div>
                ...
                <div className="flex items-center gap-2 font-mono">
                  ...
                  {!!selected.dni && (
                    <Button
                      ...
                      className="h-7 w-7"
                      onClick={async () => {
                        await navigator.clipboard.writeText(selected.dni!)
                      }}
                  ...
                  {selected.unidad_id ? (
                    <>
                      <div>
                        Unidad:{' '}
                        <span className="font-medium">
                          {unidadById.get(selected.unidad_id)?.nombre}
                        </span>
                      </div>
                      <div>
                        Zona:{' '}
                        <span className="font-medium">
                          {unidadById.get(selected.unidad_id)?.zona}
                        </span>
                      </div>
                    </>
                  ) : selected.caseta_id ? (
                    <>
                      <div>
                        Caseta:{' '}
                        <span className="font-medium">
                          {casetaById.get(selected.caseta_id)?.nombre}
                        </span>
                      </div>
                      <div>
                        Zona:{' '}
                        <span className="font-medium">
                          {
                            municipioById.get(casetaById.get(selected.caseta_id!)!.municipio_id)
                              ?.zona
                          }
                        </span>
                      </div>
